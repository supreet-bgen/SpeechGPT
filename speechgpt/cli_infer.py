...
class SpeechGPTInference:
    def __init__(
        ...
        # CodeHiFiGANVocoder(vocoder, vocoder_cfg).to(device)
        self.vocoder = CodeHiFiGANVocoder(vocoder, vocoder_cfg).to(device)

        # if device == 'auto' and torch.cuda.is_available():
        #     self.vocoder = CodeHiFiGANVocoder(vocoder, vocoder_cfg).to('cuda')
        # else:
        #     self.vocoder = CodeHiFiGANVocoder(vocoder, vocoder_cfg).to('cpu')
    ...
    def preprocess(
        ...
        for part in raw_text.split("is input:"):
            ...
            if path == '':
                pass
            if path.startswith("<sosp>"):
                processed_parts.append(part)
                continue
            elif os.path.isfile(path) and os.path.splitext(path)[-1] in [".wav", ".flac", ".mp4"]:
